...
enum Command {
    ...
    ProfileNames,
    /// Install or remove a statusline showing the active profile.
    Statusline {
        #[command(subcommand)]
        action: commands::statusline::Action,
    },
    /// (hidden) Render the statusline line. Invoked by Claude Code itself via the
...
fn dispatch_command(
    ...
    match command {
        ...
        Command::Statusline { action } => commands::statusline::run_action(action, paths, cwd),
        Command::StatuslineRender => {
